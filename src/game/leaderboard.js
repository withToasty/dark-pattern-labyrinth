document.addEventListener('DOMContentLoaded', () => {
  ...
  const fmt = (ms) => {
    ...
    return m
      ...
      : `${s}.${String(c).padStart(2, '0')}s`;
  };

  async function loadTop10() {
    const url =
      API +
      '/leaderboard?select=player_name,time_ms,traps,created_at&order=time_ms.asc,created_at.asc&limit=10';
  ...
  async function getRank(timeMs) {
    const r = await fetch(API + '/rpc/rank_for_score', {
      method: 'POST',
      headers: baseHeaders,
      body: JSON.stringify({ score_ms: timeMs }),
    });
  ...
  function renderRows(rows, host) {
    ...
    if (!rows.length) {
      ...
      e.textContent = 'まだ記録がありません。最初の記録を残せます。';
      host.appendChild(e);
      return;
    }
    rows.forEach((row, i) => {
      const item = document.createElement('div');
      item.className = 'lb-row' + (i === 0 ? ' lb-first' : '');
      ...
      time.textContent = fmt(row.time_ms);
  ...
  async function refreshBoard(host) {
    try {
      renderRows(await loadTop10(), host);
    } catch (e) {
      host.textContent = 'ランキングを読み込めませんでした。';
    }
  }
  ...
  function mount(result) {
    ...
    btn.addEventListener('click', async () => {
      if (result.validRun === false || sessionStorage.getItem('dp-run-submitted') === '1') return;
      ...
      try {
        ...
        sessionStorage.setItem('dp-run-submitted', '1');
        input.disabled = true;
        btn.disabled = true;
        btn.textContent = '登録済み';
