document.addEventListener('DOMContentLoaded',()=>{
  const state={started:false,start:0,end:0,views:0,revisits:0,traps:0,seen:new Set(),validRun:false,runId:'',trapIds:new Set(['jointrap','upgrade','pausetrap','retain1','closeTrap','reasontrap','runnerCaught','wrongLearned','callback','surveytrap','stayfinal','resetloop'])};
  let finished=false;
  ...
  const persist=()=>{try{sessionStorage.setItem('dp-run-state',JSON.stringify({start:state.start,views:state.views,revisits:state.revisits,traps:state.traps,seen:[...state.seen],validRun:state.validRun,runId:state.runId}));}catch(e){}};
  const start=(validRun=false)=>{state.started=true;state.start=Date.now();state.end=0;state.views=0;state.revisits=0;state.traps=0;state.seen.clear();state.validRun=validRun;state.runId=(crypto.randomUUID?.()||String(Date.now())+'-'+Math.random().toString(36).slice(2));finished=false;sessionStorage.setItem('dp-started','1');sessionStorage.removeItem('dp-run-submitted');persist();};
  const restore=()=>{try{const raw=sessionStorage.getItem('dp-run-state');if(!raw)return false;const saved=JSON.parse(raw);if(!saved?.start)return false;state.started=true;state.start=Number(saved.start);state.views=Number(saved.views)||0;state.revisits=Number(saved.revisits)||0;state.traps=Number(saved.traps)||0;state.seen=new Set(Array.isArray(saved.seen)?saved.seen:[]);state.validRun=!!saved.validRun;state.runId=saved.runId||'';finished=false;return true;}catch(e){return false;}};
  const record=(id)=>{
    if(!state.started||id==='start')return;
    state.views++;
    if(state.seen.has(id))state.revisits++;else state.seen.add(id);
    if(state.trapIds.has(id))state.traps++;persist();
    if(id==='trueend'&&!finished){finished=true;state.end=Date.now();persist();renderResult();}
  };
  const renderResult=()=>{
    ...
    const marker=end.querySelector('.good');marker?.after(panel);window.dispatchEvent(new CustomEvent('dp:finished',{detail:{timeMs:Math.round(state.end-state.start),views:state.views,revisits:state.revisits,traps:state.traps,seen:state.seen.size,validRun:state.validRun,runId:state.runId}}));
  };

  document.querySelector('#start a[href="#home"]')?.addEventListener('click',()=>start(true));
  document.querySelector('#trueend a[href="#start"]')?.addEventListener('click',()=>{sessionStorage.removeItem('dp-started');sessionStorage.removeItem('dp-run-state');sessionStorage.removeItem('dp-run-submitted');});

  if(idFromHash()!=='start'&&idFromHash()!=='trueend'){if(!restore())start(false);}
  record(idFromHash());
